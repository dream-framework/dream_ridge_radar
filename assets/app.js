...
function dataSeries() { const i = idx(); return i ? (($("viewSelect")?.value || 'recent') === 'long' ? i.series_long : i.series_recent) || [] : []; }
function targetLabel(key) { return [...(bundle.targets?.crash || []), ...(bundle.targets?.bull || [])].find(t => t.key === key)?.label || key || 'target'; }
function bar(value, kind) { const v = clamp(value); const cls = kind || scoreClass(v); return `<div class="bar ${cls}"><i style="width:${v}%"></i></div>`; }
...
function syncChartTabs() {
  document.querySelectorAll('.chart-tab').forEach(btn => btn.classList.toggle('active', btn.dataset.chart === activeChart));
}

function initializeControls() {
  const indices = bundle.indices || [];
  activeIndex = activeIndex || indices[0]?.key;
  $('indexSelect').innerHTML = indices.map(i => `<option value="${esc(i.key)}">${esc(i.key)} - ${esc(i.name)}</option>`).join('');
  $('indexSelect').value = activeIndex;
  const targets = [...(bundle.targets?.crash || []), ...(bundle.targets?.bull || [])];
  $('targetSelect').innerHTML = targets.map(t => `<option value="${esc(t.key)}">${esc(t.label)}</option>`).join('');
  $('targetSelect').value = targets[1]?.key || targets[0]?.key || '';
  ['indexSelect','viewSelect','targetSelect'].forEach(id => $(id).addEventListener('change', () => {
...
function renderKpis() {
  ...
  const cards = [
    ...
    ['Bull thrust', `${num(c.bull_score,1)}`, 'coherent upside pressure'],
    ...
    ['Score target', targetLabel($('targetSelect').value), 'backtest panel below']
  ];
...
function renderCurrentRead() {
  const i = idx(); const c = i?.current || {};
  const tiles = [
    ['Risk', `${num(c.risk_score,1)} / 100`, 'dust + strain + lambda_q instability'],
    ['Bull', `${num(c.bull_score,1)} / 100`, 'upward ridge pressure, separately scored'],
  ...
  $('currentRead').innerHTML = tiles.map(t => `<div class="metric-tile"><span>${esc(t[0])}</span><b>${esc(t[1])}</b><p>${esc(t[2])}</p></div>`).join('');
}
...
function renderNarrative() {
  ...
  $('globalNarrative').innerHTML = `<strong>Narrate.</strong> ${esc(i?.narrative || 'No selected-index narrative.')}<br><br><strong>Global leaders:</strong> risk ${esc(risk)}; bull ${esc(bull)}. Median risk ${num(s.median_risk,1)}, median bull ${num(s.median_bull,1)}.`;
}

function drawChart() {
  const i = idx(); if (!i) return;
  const data = dataSeries();
  $('chartTitle').textContent = `${i.key} ${activeChart === 'ridge' ? 'ridge theater' : activeChart === 'dust' ? 'dust cloud' : activeChart === 'lambda' ? 'lambda_q flicker' : 'event scores'}`;
  $('chartSubtitle').textContent = activeChart === 'ridge' ? 'close, retained ridge, dust envelope, correction risk, bull thrust'
    ...
    : `${targetLabel($('targetSelect').value)} score context`;
  $('chartNarrative').textContent = i.narrative || 'Retained ridge is the low-frequency structure; dust cloud is the operational residual around it; lambda_q flicker marks unstable S2 probe scale.';
  if (!window.echarts) { drawFallbackSvg(data); return; }
  if (!chart) chart = echarts.init($('mainChart'));
  const light = document.documentElement.classList.contains('light');
  const p = light ? { text:'#162331', muted:'#617181', grid:'rgba(33,51,68,.14)', price:'#0878bd', ridge:'#14844a', risk:'#c83434', bull:'#168d54', dust:'#9c6700', lambda:'#6741d9', env:'rgba(8,120,189,.12)' }
  ...
  const axis = { type:'category', data:dates, axisLine:{lineStyle:{color:p.grid}}, axisLabel:{color:p.muted, hideOverlap:true, fontSize:10} };
  const base = { animation:false, textStyle:{color:p.text, fontSize:11}, tooltip:{trigger:'axis'}, legend:{top:0,textStyle:{color:p.muted,fontSize:10}, itemWidth:12,itemHeight:7}, grid:{left:46,right:36,top:34,bottom:26}, xAxis:axis };
  let option;
  if (activeChart === 'ridge') {
    option = { ...base, yAxis:[{type:'log', axisLabel:{color:p.muted,fontSize:10}, splitLine:{lineStyle:{color:p.grid}}},{type:'value',min:0,max:100,axisLabel:{color:p.muted,fontSize:10},splitLine:{show:false}}], series:[
    ...
    option = { ...base, yAxis:[{type:'value',axisLabel:{color:p.muted,fontSize:10},splitLine:{lineStyle:{color:p.grid}}},{type:'value',min:0,max:100,axisLabel:{color:p.muted,fontSize:10},splitLine:{show:false}}], series:[
    ...
    option = { ...base, yAxis:[{type:'value',name:'lambda/beta',axisLabel:{color:p.muted,fontSize:10},splitLine:{lineStyle:{color:p.grid}}},{type:'value',name:'flicker',min:0,max:100,axisLabel:{color:p.muted,fontSize:10},splitLine:{show:false}}], series:[
    ...
    option = { ...base, yAxis:[{type:'value',min:0,max:100,axisLabel:{color:p.muted,fontSize:10},splitLine:{lineStyle:{color:p.grid}}},{type:'value',axisLabel:{color:p.muted,fontSize:10},splitLine:{show:false}}], series:[
      {name:'Risk score', type:'line', data:data.map(d=>d.risk_score), showSymbol:false, lineStyle:{width:1.5,color:p.risk}},
      {name:'Bull score', type:'line', data:data.map(d=>d.bull_score), showSymbol:false, lineStyle:{width:1.2,color:p.bull}},
...
function drawFallbackSvg(data) {
  ...
  const path = pts.map(([x,y], k) => `${k?'L':'M'} ${20 + x*(w-40)/Math.max(1,pts.length-1)} ${h-20 - y*(h-40)/100}`).join(' ');
  el.innerHTML = `<svg width="100%" height="100%" viewBox="0 0 ${w} ${h}"><rect width="${w}" height="${h}" fill="transparent"/><path d="${path}" fill="none" stroke="currentColor" stroke-width="2"/></svg>`;
}
...
function panel(title, hint, body) { return `<section class="subpanel"><h3>${esc(title)}</h3>${hint ? `<div class="hint">${esc(hint)}</div>` : ''}${body}</section>`; }
...
function matrixTable(limit) {
  const rows = [...(bundle.indices || [])].sort((a,b)=>(b.current?.risk_score||0)-(a.current?.risk_score||0)).slice(0, limit || 999);
  return `<table><thead><tr><th>Index</th><th>State</th><th class="num">Risk</th><th class="num">Bull</th><th class="num">Dust</th><th class="num">Strain</th><th class="num">lambda_q</th></tr></thead><tbody>` + rows.map(i => {
    const c=i.current||{}; return `<tr><td><button class="pill-button" onclick="selectIndex('${esc(i.key)}')">${esc(i.key)}</button></td><td><span class="${stateClass(c.state)}">${esc(c.state)}</span></td><td class="num bar-cell">${num(c.risk_score,0)}${bar(c.risk_score)}</td><td class="num bar-cell">${num(c.bull_score,0)}${bar(c.bull_score,'good')}</td><td class="num">${num(c.dust_z,2)}</td><td class="num">${num(c.excursion,2)}</td><td class="num">${num(c.lambda_q,1)}</td></tr>`;
  }).join('') + `</tbody></table>`;
}
window.selectIndex = function(key) { activeIndex = key; $('indexSelect').value = key; renderAll(); };

function leaderTable(kind) {
  const field = kind === 'bull' ? 'bull_score' : 'risk_score';
  const rows = [...(bundle.indices || [])].sort((a,b)=>(b.current?.[field]||0)-(a.current?.[field]||0)).slice(0,10);
  return `<table><thead><tr><th>Index</th><th>Name</th><th class="num">${kind}</th><th class="num">lambda flicker</th></tr></thead><tbody>` + rows.map(i => `<tr><td>${esc(i.key)}</td><td>${esc(i.name)}</td><td class="num">${num(i.current?.[field],1)}</td><td class="num">${num(i.current?.lambda_flicker_score,1)}</td></tr>`).join('') + `</tbody></table>`;
}
function backtestTable(indexKey) {
  ...
  return `<table><thead><tr><th>Index</th><th>Model</th><th class="num">AUC</th><th class="num">Prec.</th><th class="num">Recall</th><th class="num">False</th><th class="num">Events</th></tr></thead><tbody>` + rows.map(r => `<tr><td>${esc(r.index)}</td><td>${esc(r.model)}</td><td class="num">${num(r.auc,3)}</td><td class="num">${pct(r.precision,1)}</td><td class="num">${pct(r.recall,1)}</td><td class="num">${pct(r.false_alert_rate,1)}</td><td class="num">${esc(r.events)}</td></tr>`).join('') + `</tbody></table>`;
}
function caseTable(indexKey, limit) {
  const kind = ($('targetSelect').value || '').startsWith('up_') ? 'bull' : 'crash';
  ...
  return `<table><thead><tr><th>Window</th><th>Index</th><th class="num">Move</th><th class="num">Pre</th><th class="num">During</th><th>First warning</th><th class="num">Lead</th></tr></thead><tbody>` + rows.map(r => `<tr><td>${esc(r.window)}</td><td>${esc(r.index)}</td><td class="num ${Number(r.realized_move)<0?'bad':'good'}">${pct(r.realized_move,1)}</td><td class="num">${num(r.max_pre_score,1)}</td><td class="num">${num(r.max_during_score,1)}</td><td>${esc(r.first_warning || '-')}</td><td class="num">${r.lead_days == null ? '-' : `${esc(r.lead_days)}d`}</td></tr>`).join('') + `</tbody></table>`;
}
function lambdaTable(limit) {
  const rows = (idx()?.s2_tail || []).slice(-limit).reverse();
  return `<table><thead><tr><th>Date</th><th class="num">lambda_q</th><th class="num">Beta</th><th class="num">R2</th><th class="num">Delta BIC</th><th class="num">Flicker</th></tr></thead><tbody>` + rows.map(r => `<tr><td>${esc(r.date)}</td><td class="num">${num(r.lambda_q,2)}</td><td class="num">${num(r.beta,3)}</td><td class="num">${num(r.r2,3)}</td><td class="num">${num(r.delta_bic_vs_d1,1)}</td><td class="num">${num(r.lambda_flicker_score,1)}</td></tr>`).join('') + `</tbody></table>`;
}
function dustRankTable() {
  const rows = [...(bundle.indices || [])].sort((a,b)=>(b.current?.dust_z||0)-(a.current?.dust_z||0));
  return `<table><thead><tr><th>Index</th><th class="num">Dust z</th><th class="num">Accel</th><th class="num">Strain</th><th class="num">Risk</th></tr></thead><tbody>` + rows.map(i=>`<tr><td>${esc(i.key)}</td><td class="num">${num(i.current?.dust_z,2)}</td><td class="num">${num(i.current?.dust_accel_z,2)}</td><td class="num">${num(i.current?.excursion,2)}</td><td class="num">${num(i.current?.risk_score,1)}</td></tr>`).join('') + `</tbody></table>`;
}
function healthTable() {
  return `<table><thead><tr><th>Index</th><th>Status</th><th>Source</th><th>Message</th></tr></thead><tbody>` + (bundle.health || []).map(r => `<tr><td>${esc(r.index)}</td><td>${esc(r.status)}</td><td>${esc(r.source)}</td><td>${esc(r.message)}</td></tr>`).join('') + `</tbody></table>`;
}
function overviewStory() {
  const s = bundle.summary || {}, i = idx(), c = i?.current || {};
  return `<div class="small-story"><div class="callout">${esc(i?.key || '')}: ${esc(c.state || 'NA')} - risk ${num(c.risk_score,1)}, bull ${num(c.bull_score,1)}.</div><div>The useful question is not next-bar direction. It is whether retained market structure is stable, thickening, or breaking.</div><div>Single-screen rule: green/yellow/red, chart, selected read, matrix, scorecard, and event lab are all visible without page scroll on desktop.</div><div>Global read: ${esc(s.global_state || 'NA')}; median dust z ${num(s.median_dust_z,2)}, median lambda flicker ${num(s.median_lambda_flicker,1)}.</div></div>`;
}
function methodNote() {
  return `<ul class="audit-list"><li>Ridge = low-frequency retained market structure.</li><li>Dust cloud = operational residual around the ridge, not revived S1.</li><li>lambda_q flicker = instability of rolling S2 coherence-scale extraction.</li><li>Risk score combines dust thickening, pullback failure, ridge flattening, and lambda_q instability.</li></ul>`;
}
function caveats() {
